...
from keras.layers import Input, concatenate, Conv2D, Dense, MaxPooling2D, Activation, UpSampling2D, BatchNormalization, Flatten, LeakyReLU, GlobalAveragePooling2D
...
def get_advanced_unet_512(input_shape=(512, 512, 1),
    ...
    model = Model(inputs=inputs, outputs=[segmentation, ep_center_x, ep_center_y, ep_axis_a, ep_axis_b, ep_angle_sin, ep_angle_cos, ep_hc])
...
#
def get_opt_advanced_unet_512(input_shape=(512, 512, 1),
                 num_classes=1):
    inputs = Input(shape=input_shape)
    # 512x512x1
